...
# Phase 3.1 (kimi-code-cli): user-scope inline block at the top of
# ~/.kimi-code/AGENTS.md (kimi-code's documented global memory)
def test_kimi_code_cli_user_inline(run, env_for, fake_home, src_file):
    r = run(["--agent", "kimi-code-cli", "--scope", "user", "--yes",
             "--source", str(src_file)], env=env_for(fake_home))
    assert r.returncode == 0, r.stdout + r.stderr
    data = (fake_home / ".kimi-code" / "AGENTS.md").read_bytes()
    assert data.startswith(B)
    assert b"# RULES\nrules body line\n" in data
    assert data.endswith(E)
    assert data.count(b"<!-- BEGIN behave ") == 1
...
# Phase 3.1 (kimi-code-cli): a pre-existing ~/.kimi-code/AGENTS.md keeps
# its content below the newly prepended block
def test_kimi_code_cli_user_inline_preserves_existing(run, env_for,
                                                       fake_home,
                                                       src_file):
...
# Phase 3.1 (kimi-code-cli): project scope rides the shared ./AGENTS.md
# family block (kimi-code also reads AGENTS.md per directory)
def test_kimi_code_cli_project_shared_agents_md(run, env_for, fake_home,
                                                proj, src_file):
    env = env_for(fake_home)
    r = run(["--agent", "kimi-code-cli", "--scope", "project",
             "--project-dir", str(proj), "--yes", "--json", "--source",
             str(src_file)], env=env)
    assert r.returncode == 0, r.stdout + r.stderr
    payload = json.loads(r.stdout)
    served = payload["targets"][0]["agent"].split(",")
    assert "kimi-code-cli" in served
    assert "codex" in served  # the shared block, not a kimi-only one
    data = (proj / "AGENTS.md").read_bytes()
    assert data.startswith(B)
    assert b"# RULES\nrules body line\n" in data
    assert data.count(b"<!-- BEGIN behave ") == 1
...
# Phase 8 (kiro-cli): project scope rides the shared ./AGENTS.md family
# block (root AGENTS.md is "always included" on all Kiro surfaces)
def test_kiro_cli_project_shared_agents_md(run, env_for, fake_home, proj,
                                           src_file):
    env = env_for(fake_home)
    r = run(["--agent", "kiro-cli", "--scope", "project",
             "--project-dir", str(proj), "--yes", "--json", "--source",
             str(src_file)], env=env)
    assert r.returncode == 0, r.stdout + r.stderr
    payload = json.loads(r.stdout)
    served = payload["targets"][0]["agent"].split(",")
    assert "kiro-cli" in served
    assert "codex" in served  # the shared block, not a kiro-only one
    data = (proj / "AGENTS.md").read_bytes()
    assert data.startswith(B)
    assert data.count(b"<!-- BEGIN behave ") == 1
...
# Phase 9 (hermes-agent): user-scope inline block at the top of
# ~/.hermes/SOUL.md (the identity file, loaded every session)
def test_hermes_agent_user_inline(run, env_for, fake_home, src_file):
    r = run(["--agent", "hermes-agent", "--scope", "user", "--yes",
             "--source", str(src_file)], env=env_for(fake_home))
    assert r.returncode == 0, r.stdout + r.stderr
    data = (fake_home / ".hermes" / "SOUL.md").read_bytes()
    assert data.startswith(B)
    assert b"# RULES\nrules body line\n" in data
    assert data.endswith(E)
    assert data.count(b"<!-- BEGIN behave ") == 1
...
# Phase 9 (hermes-agent): a pre-existing ~/.hermes/SOUL.md keeps its
# content below the newly prepended block
def test_hermes_agent_user_inline_preserves_existing(run, env_for,
                                                     fake_home,
                                                     src_file):
    target = fake_home / ".hermes" / "SOUL.md"
    target.parent.mkdir(parents=True)
    target.write_bytes(b"MY SOUL\n")
    r = run(["--agent", "hermes-agent", "--scope", "user", "--yes",
             "--source", str(src_file)], env=env_for(fake_home))
    assert r.returncode == 0, r.stdout + r.stderr
    data = target.read_bytes()
    assert data.startswith(B)
    assert b"MY SOUL" in data
    assert data.count(b"<!-- BEGIN behave ") == 1


# Phase 9 (hermes-agent): project scope rides the shared ./AGENTS.md
# family block (AGENTS.md is in its project chain)
def test_hermes_agent_project_shared_agents_md(run, env_for, fake_home,
                                               proj, src_file):
    env = env_for(fake_home)
    r = run(["--agent", "hermes-agent", "--scope", "project",
             "--project-dir", str(proj), "--yes", "--json", "--source",
             str(src_file)], env=env)
    assert r.returncode == 0, r.stdout + r.stderr
    payload = json.loads(r.stdout)
    served = payload["targets"][0]["agent"].split(",")
    assert "hermes-agent" in served
    assert "codex" in served  # the shared block, not a hermes-only one
    data = (proj / "AGENTS.md").read_bytes()
    assert data.startswith(B)
    assert data.count(b"<!-- BEGIN behave ") == 1


# Phase 9 (aider-desk): project scope rides the shared ./AGENTS.md
# family block (agent mode auto-loads the root AGENTS.md)
def test_aider_desk_project_shared_agents_md(run, env_for, fake_home,
                                             proj, src_file):
    env = env_for(fake_home)
    r = run(["--agent", "aider-desk", "--scope", "project",
             "--project-dir", str(proj), "--yes", "--json", "--source",
             str(src_file)], env=env)
    assert r.returncode == 0, r.stdout + r.stderr
    payload = json.loads(r.stdout)
    served = payload["targets"][0]["agent"].split(",")
    assert "aider-desk" in served
    assert "codex" in served  # the shared block, not an aider-desk-only one
    data = (proj / "AGENTS.md").read_bytes()
    assert data.startswith(B)
    assert data.count(b"<!-- BEGIN behave ") == 1
...
# Phase 10 (jcode): user-scope inline block at the top of
# ~/.jcode/prompt-overlay.md (the "Global Prompt Overlay")
def test_jcode_user_inline(run, env_for, fake_home, src_file):
    r = run(["--agent", "jcode", "--scope", "user", "--yes",
             "--source", str(src_file)], env=env_for(fake_home))
    assert r.returncode == 0, r.stdout + r.stderr
    data = (fake_home / ".jcode" / "prompt-overlay.md").read_bytes()
    assert data.startswith(B)
    assert b"# RULES\nrules body line\n" in data
    assert data.endswith(E)
    assert data.count(b"<!-- BEGIN behave ") == 1
...
# Phase 10 (jcode): a pre-existing ~/.jcode/prompt-overlay.md keeps
# its content below the newly prepended block
def test_jcode_user_inline_preserves_existing(run, env_for,
                                              fake_home, src_file):
    target = fake_home / ".jcode" / "prompt-overlay.md"
    target.parent.mkdir(parents=True)
    target.write_bytes(b"MY NOTES\n")
    r = run(["--agent", "jcode", "--scope", "user", "--yes",
             "--source", str(src_file)], env=env_for(fake_home))
...
# Phase 10 (jcode): project scope rides the shared ./AGENTS.md
# family block (the default chain)
def test_jcode_project_shared_agents_md(run, env_for, fake_home,
                                        proj, src_file):
    env = env_for(fake_home)
    r = run(["--agent", "jcode", "--scope", "project",
             "--project-dir", str(proj), "--yes", "--json", "--source",
             str(src_file)], env=env)
    assert r.returncode == 0, r.stdout + r.stderr
    payload = json.loads(r.stdout)
    served = payload["targets"][0]["agent"].split(",")
    assert "jcode" in served
    assert "codex" in served  # the shared block, not a jcode-only one
    data = (proj / "AGENTS.md").read_bytes()
    assert data.startswith(B)
    assert data.count(b"<!-- BEGIN behave ") == 1
