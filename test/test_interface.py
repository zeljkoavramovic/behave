...
def test_agent_menu_all_shown_installs_only_detected(run, env_for,
    ...
    assert not (fake_home / ".junie").exists()
    assert "Traceback" not in combined
...
# Phase 8 (antigravity): promoted agent - --list shows antigravity
# detected without the "(no install support)" note
def test_antigravity_list_install_support(run, env_for, fake_home):
...
# Phase 8 (qoder): promoted agent - --list shows qoder detected without
# the "(no install support)" note
def test_qoder_list_install_support(run, env_for, fake_home):
    (fake_home / ".qoder").mkdir()
    r = run(["--list"], env=env_for(fake_home))
    assert r.returncode == 0, r.stdout + r.stderr
    lines = [ln for ln in r.stdout.splitlines()
             if ln.strip().split() and ln.strip().split()[0] == "qoder"]
    assert lines, r.stdout
    assert "(no install support)" not in lines[0]


# Phase 8 (grok): promoted agent - --list shows grok detected without
# the "(no install support)" note
def test_grok_list_install_support(run, env_for, fake_home):
    (fake_home / ".grok").mkdir()
    r = run(["--list"], env=env_for(fake_home))
    assert r.returncode == 0, r.stdout + r.stderr
    lines = [ln for ln in r.stdout.splitlines()
             if ln.strip().split() and ln.strip().split()[0] == "grok"]
    assert lines, r.stdout
    assert "(no install support)" not in lines[0]


# Phase 8 batch 2 (mistral-vibe): promoted agent - --list shows
# mistral-vibe detected without the "(no install support)" note
def test_mistral_vibe_list_install_support(run, env_for, fake_home):
    (fake_home / ".vibe").mkdir()
    r = run(["--list"], env=env_for(fake_home))
    assert r.returncode == 0, r.stdout + r.stderr
    lines = [ln for ln in r.stdout.splitlines()
             if ln.strip().split() and ln.strip().split()[0] == "mistral-vibe"]
    assert lines, r.stdout
    assert "(no install support)" not in lines[0]


# Phase 8 batch 2 (rovodev): promoted agent - --list shows rovodev
# detected without the "(no install support)" note
def test_rovodev_list_install_support(run, env_for, fake_home):
...
# Phase 8 batch 2 (trae-cn): promoted agent - --list shows trae-cn
# detected without the "(no install support)" note
def test_trae_cn_list_install_support(run, env_for, fake_home):
...
# Phase 8 batch 2 (antigravity-cli): promoted agent - --list shows
# antigravity-cli detected without the "(no install support)" note
def test_antigravity_cli_list_install_support(run, env_for, fake_home):
...
# Phase 9 (command-code): promoted agent - --list shows command-code
# detected without the "(no install support)" note
def test_command_code_list_install_support(run, env_for, fake_home):
...
# Phase 10 (jcode): promoted agent - --list shows jcode detected
# without the "(no install support)" note
def test_jcode_list_install_support(run, env_for, fake_home):
...
# Phase 8 batch 5 (kimchi): promoted agent - --list shows kimchi
# detected without the "(no install support)" note
def test_kimchi_list_install_support(run, env_for, fake_home):
...
# Phase 8 batch 5 (pochi): promoted agent - --list shows pochi
# detected without the "(no install support)" note
def test_pochi_list_install_support(run, env_for, fake_home):
...
# Phase 8 batch 5 (reasonix): promoted agent - --list shows reasonix
# detected without the "(no install support)" note
def test_reasonix_list_install_support(run, env_for, fake_home):
